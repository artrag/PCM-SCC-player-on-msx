...
instruction_text:	
		db "Press any key from C to Z",0
scc_slot_text:
		db	"Scc slot: ",0
rom_slot_text:
		db	"Rom slot: ",0
...
; the entire wave table starting at sample 'a'
;-------------------------------------
;-------------------------------------
; write the first NSmp samples
;-------------------------------------
NSmp:  equ 21
...
;-------------------------------------
; write the last 32-NSmp samples
;-------------------------------------
myldir:
        ld      bc,NSmp
        add     hl,bc
...
ReplayerUpdate:
		...
		ld      a,00001111b     ; channels 1-4 active
        ld      (988Fh),a
...
SccInit:
        ...
        ld      (9886h),hl
		; ld		hl,0
        ld      (9888h),hl		; experiment on ch 5
...
Period:             equ VarBase + 4
NumBlocksToPlay:    equ VarBase + 6
SccSfxOn:           equ VarBase + 8
Keys:               equ VarBase + 10

slotvar:            equ VarBase + 12
slotram:            equ VarBase + 14
SCC:            	equ VarBase + 16
curslot:            equ VarBase + 18
